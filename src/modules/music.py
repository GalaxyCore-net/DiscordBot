...
logger = logging.getLogger("BASE / MUSIC  : ")
...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    async def music_voice(self, user, before, after):
        ...
        if after.channel is None and user.id == self.bot.user.id:
            try:
                ...
                logger.error(f"Failed to get guild id {user.guild.id}")
    ...
    async def queue(self, msg, song):
        ...
        if data['queue']:
            ...
            return await msg.send(f"Playlist {data['title']} zur Warteschleide hinzugefügt")
        self.player[msg.guild.id]['queue'].append(
            {'title': title, 'author': msg})
        return await msg.send(f"{title} zur Warteschleide hinzugefügt".title())
    ...
    async def loop_song(self, msg):
        ...
        try:
            ...
            logger.error(Error)  # NOTE: output back the error for later debugging
    ...
    async def done(self, msg, msgId: int = None):
        ...
        if msgId:
            try:
                ...
                logger.warning("Failed to get the message")
    ...
    async def start_song(self, msg, song):
        ...
        emb = discord.Embed(colour=self.random_color, title='Spielt jetzt:',
                            description=download.title, url=download.url)
        emb.set_thumbnail(url=download.thumbnail)
        emb.set_footer(
            text=f'{msg.author.display_name}', icon_url=msg.author.avatar_url)
        loop = asyncio.get_event_loop()
    ...
    async def play(self, msg, *, song):
        if msg.channel.id != config["music-commands"]:
            return
        """
        ...
        try:
            ...
            if msg.author.voice is not None and msg.voice_client is not None:
                ...
                await msg.voice_client.disconnect()
            await msg.send("Song nicht gefunden")
    ...
    async def before_play(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send('**Du musst in einem Voicechannel sein, um Musik spielen zu können**'.title())
        ...
        if msg.voice_client.channel != msg.author.voice.channel:
            ...
            if self.player[msg.guild.id]['queue']:
                ...
                return await msg.send("**Du musst in einem Voicechannel sein, um Musik spielen zu können**")
    ...
    async def repeat(self, msg):
        ...
        if msg.guild.id in self.player:
            ...
            return await msg.send("Aktuell wird nichts abgespielt.")
        return await msg.send("Aktuell wird nichts abgespielt.")
    ...
    async def reset(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send(f"**{msg.author.display_name}, Aktuell wird nichts abgespielt.**")

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send(f"**{msg.author.display_name}, Dazu musst du im selben Voicekanal sein wie ich.**")

        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Aktuell wird nichts abgespielt.**".title(), delete_after=25)
    ...
    async def skip(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("**Aktuell wird keine Musik abgespielt**".title(), delete_after=60)

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send("Dazu musst du im selben Voicechannel sein wie der Bot")

        if not self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Nichts zum überspringen**".title(), delete_after=60)
    ...
    async def stop(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("Ich bin nicht in einem Voicekanal")

        if msg.author.voice is None:
            return await msg.send("Dazu musst du im selben Voicekanal sein wie ich")
    ...
    async def leave(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send("Dazu musst du im selben Channel sein wie ich")
    ...
    async def pause(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is True:
                return await msg.send("Der Song ist bereits pausiert")
    ...
    async def resume(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is False:
                return await msg.send("Der Song spielt bereits")
    ...
    async def _queue(self, msg):
        ...
        if msg.voice_client is not None:
            if msg.guild.id in self.player:
                if self.player[msg.guild.id]['queue']:
                    ...
                    emb.set_footer(
                        text=f'{msg.author.name}', icon_url=msg.author.avatar_url)
        ...
        return await msg.send("Keine Songs in der Warteschleife")
    ...
    async def song_info(self, msg):
        ...
        return await msg.send(f"**Aktuell wird nichts abgespielt**".title(), delete_after=30)
    ...
    async def join(self, msg, *, channel: discord.VoiceChannel = None):
        ...
        """
        if msg.voice_client is not None:
            return await msg.send(f"Ich bin bereits in einem Voicechanne\nMeintest du {msg.prefix}moveTo")
    ...
    async def before_join(self, msg):
        if msg.channel.id != config["music-commands"]:
            return
        if msg.author.voice is None:
            return await msg.send("Du bist nicht in einem Voicechannel")
    ...
    async def join_error(self, msg, error):
        ...
        if error.args[0] == 'Command raised an exception: Exception: playing':
            return await msg.send("**Du musst im selben Channel wie ich sein um einen Song abspielen zu können**".title())
    ...
    async def volume(self, msg, vol: int):
        ...
        return await msg.send("**Du musst im selben Channel wie ich sein um einen Song abspielen zu können**".title(), delete_after=30)
    ...
    async def download(self, ctx, *, song):
        if ctx.channel.id != config["music-commands"]:
            return
        """
        ...
        try:
            with youtube_dl.YoutubeDL(ytdl_download_format_options) as ydl:
                ...
                embed = discord.Embed(
                    title="", description="Warte einen Moment")
                await ctx.send(embed=embed, delete_after=30)
                await ctx.send(file=discord.File(filename))
                os.remove(filename)
        except (youtube_dl.utils.ExtractorError, youtube_dl.utils.DownloadError):
            embed = discord.Embed(title="Song konnte nicht gedownloaded werden", description=("Song:"+song))
            await ctx.send(embed=embed)

    @volume.error
    async def volume_error(self, msg,error):
        if isinstance(error, commands.MissingPermissions):
            return await msg.send("Das darfst du nicht!", delete_after=30)
